...
from pearbook.settings import SECRET_KEY
from operator import itemgetter
from pearbook.common.code import *
from pearbook.common.custom import CommonPagination
from pearbook.rbac.models import Menu

from pearbook.rbac.serializers.user_serializer import UserListSerializer, UserCreateSerializer, UserModifySerializer, UserInfoListSerializer
from pearbook.rbac.serializers.menu_serializer import MenuSerializer
...
class UserAuthView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        has_email = UserProfile.objects.get(email=email)
        # 如果用户存在
        if has_email and password == has_email.password:
            payload = jwt_payload_handler(has_email)
            token = jwt.encode(payload, SECRET_KEY)
            user = UserProfile.objects.filter(email=email, is_active=True)
            # name = user.values('name')[0].values()
            # email = user.values('email')[0].values()
            data = {
                # 'name': str(list(name)[0]),
                # 'email': str(list(email)[0]),
                'user': user.values('username', 'email', 'is_staff')[0],
                'token': str(token)
            }
            return JsonResponse(data, safe=False)
        else:
            # return HttpResponse('用户名或密码错误!', status=BAD)
...
class UserInfoView(APIView):
    ...
    def get(self, request):
        if request.user.id is not None:
            ...
            return HttpResponse(data, status=OK)
        else:
            return HttpResponse('请登录后访问!', status=FORBIDDEN)
...
class UserBuildMenuView(APIView):
    ...
    def get(self, request):
        if request.user.id is not None:
            ...
            return HttpResponse(menu_data, status=OK)
        else:
            return HttpResponse('请登录后访问!',status=FORBIDDEN)
...
class UserViewSet(ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        email = request.data.get('email')
        has_email = UserProfile.objects.filter(email=email,username=username)
        # 如果已经存在用户
        if has_email:
            return HttpResponse('已经存在用户')
        # 没有用户
        else:
            # 保存到数据库
    ...
    def set_password(self, request, pk=None):
        ...
        if 'admin' in perms or 'user_all' in perms or request.user.is_superuser:
            ...
            if new_password1 == new_password2:
                ...
                return HttpResponse('新密码两次输入不一致!', status=status.HTTP_400_BAD_REQUEST)
        else:
            ...
            if check_password(old_password, user.password):
                ...
                return HttpResponse('旧密码错误!', status=status.HTTP_400_BAD_REQUEST)
